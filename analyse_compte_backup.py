...
def charger_comptes() -> list:
    ...
    # Le fichier peut être encodé en Windows-1252 (cp1252) sur les machines Windows
...
def charger_csv(annee: str, mois: str) -> pd.DataFrame | None:
    """Charge le CSV sauvegardé pour une période, ou None si absent."""
    chemin = _chemin_csv(annee, mois)
    if os.path.exists(chemin):
        try:
            df = pd.read_csv(chemin, sep=";", encoding="utf-8-sig")
            # S'assurer que Date et Libellé sont bien des strings
            # (les NaN deviennent float à la lecture CSV)
            ...
            # Supprimer la catégorie pour forcer une re-catégorisation
            # — permet de détecter les chèques et virements opaques
            # même si le CSV a été sauvegardé avec une ancienne version
...
def csv_est_complet(annee: str, mois: str) -> bool:
    """Retourne True si le CSV existe et ne contient aucune ligne non catégorisée."""
    df = charger_csv(annee, mois)
    if df is None:
        return False
    cats_incompletes = {"À catégoriser", "À catégoriser (chèque)"}
    return not df["Catégorie"].isin(cats_incompletes).any()
...
def _est_virement_opaque(libelle: str) -> bool:
    """Virement dont le tronc distinctif est vide ou trop court.
    Ex : 'VIR INST FACTURE 250119' → tronc vide → opaque comme un chèque.
    """
    lib = libelle.strip()   # supprimer espaces parasites
...
def categoriser(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Appliquer les règles mots-clés sur les lignes NON manuelles uniquement
...
# Navigation automatique selon l'état après extraction
_TAB_INDEX = {
    "comptes":       0,
    "fichiers":      1,
    "categorisation": 2,
...
# Injecter du JS pour switcher d'onglet automatiquement
if _goto in _TAB_INDEX:
    idx = _TAB_INDEX[_goto]
    st.components.v1.html(f"""
        <script>
        // Attendre que les onglets soient rendus puis cliquer sur le bon
        function switchTab() {{
        ...
        if st.button("🔄 Extraire et consolider", key="btn_extraire"):
            ...
            with st.spinner("Chargement en cours…"):
                for path in fichiers_ok:
                    # Chercher si un CSV existe pour ce fichier spécifique
                    # Un CSV est valide pour ce fichier si sa Source correspond
                    nom_source = os.path.basename(path)
                    df_depuis_csv = None

                    if periode_unique:
                        annee_u, mois_u = annees_sel[0], mois_sel[0]
                        df_csv_periode = charger_csv(annee_u, mois_u)
                        if df_csv_periode is not None and "Source" in df_csv_periode.columns:
                            # Extraire les lignes de ce fichier depuis le CSV
                            df_source = df_csv_periode[
                                df_csv_periode["Source"] == nom_source
                            ].copy()
                            if not df_source.empty:
                                # Supprimer la catégorie pour re-catégoriser
                                # avec les règles actuelles
                                df_source = df_source.drop(
                                    columns=["Catégorie"], errors="ignore"
                                )
                                df_depuis_csv = df_source
                                st.caption(f"💾 {nom_source} → rechargé depuis le CSV")

                    if df_depuis_csv is not None:
                        dfs.append(df_depuis_csv)
                    else:
                        # Pas de CSV pour ce fichier → extraire depuis le PDF
            ...
            if dfs:
                ...
                # Mémoriser la période pour la sauvegarde CSV
                st.session_state["_annees_sel"] = annees_sel
                st.session_state["_mois_sel"]   = mois_sel

                # Compter ce qui reste à catégoriser
                cats_incompletes = {"À catégoriser", "À catégoriser (chèque)"}
                nb_a_cat = df_cat["Catégorie"].isin(cats_incompletes).sum()

                if nb_a_cat == 0:
                    # Tout est déjà catégorisé → guider vers Analyse
                    st.session_state["_goto_tab"] = "analyse"
                    st.success(f"✅ {len(consolidated)} transactions chargées — tout est catégorisé !")
                    st.info("👉 Rendez-vous dans l'onglet **📊 Analyse** pour consulter vos résultats.")
                else:
                    # Il reste des choses à catégoriser → guider vers Catégorisation
                    st.session_state["_goto_tab"] = "categorisation"
                    st.success(f"✅ {len(consolidated)} transactions chargées.")
                    st.warning(f"👉 {nb_a_cat} transaction(s) à catégoriser — rendez-vous dans l'onglet **🏷️ Catégorisation**.")
                st.rerun()
            else:
...
with tab_categories:
    ...
    # ── Virements opaques — section séparée ─────────────────────────────────
    ...
    if not vir_opaques.empty:
        ...
        st.warning(
            "Ces virements ne contiennent pas assez d'information pour être "
            "catégorisés automatiquement. Indiquez leur objet."
        )
        ...
        for _, row_vir in vir_opaques.iterrows():
            ...
            with c1:
                st.write(f"❓ **{row_vir['Libellé']}**")
            with c2:
                sens = "💸" if row_vir["Débit"] > 0 else "💰"
                montant = row_vir["Débit"] if row_vir["Débit"] > 0 else row_vir["Crédit"]
                st.caption(f"{sens} {montant:,.2f} €")
            with c3:
                st.caption(f"Date : {row_vir['Date']}")
            with c4:
                choix_vir = st.selectbox(
                    "cat",
                    ["— Choisir —"] + cats_dispo_vir + ["➕ Nouvelle catégorie…"],
                    key=f"vir_{row_vir.name}",
                    label_visibility="collapsed",
                )
                if choix_vir == "➕ Nouvelle catégorie…":
                    nouvelle_vir = st.text_input(
                        "Nom", key=f"new_vir_{row_vir.name}",
                        placeholder="Nouvelle catégorie",
                        label_visibility="collapsed",
                    )
                    if nouvelle_vir.strip():
                        assign_vir[row_vir.name] = ("new", nouvelle_vir.strip())
        ...
        if st.button("✅ Valider les virements", key="btn_valider_vir"):
            ...
            for idx_vir, (kind, valeur) in assign_vir.items():
                ...
                # Mémoriser par date+libellé comme les chèques
                ...
                st.session_state["combined_df"].loc[idx_vir, "Catégorie"] = cat_finale
            if assign_vir:
                sauvegarder_categories()
                sauvegarder_cheques()
                st.success(f"{len(assign_vir)} virement(s) catégorisé(s) et mémorisé(s).")
                st.rerun()
        st.divider()
    ...
    if not cheques.empty:
        ...
        if st.button("✅ Valider les chèques", key="btn_valider_chq"):
            ...
            if assign_chq:
                sauvegarder_categories()
                sauvegarder_cheques()
                # CSV auto si tout est catégorisé après les chèques
                df_apres = st.session_state["combined_df"]
                cats_incompletes = {"À catégoriser", "À catégoriser (chèque)"}
                nb_restant = df_apres["Catégorie"].isin(cats_incompletes).sum()
                if nb_restant == 0:
                    f_annees = st.session_state.get("_annees_sel", st.session_state.get("f_annees", []))
                    f_mois   = st.session_state.get("_mois_sel",   st.session_state.get("f_mois",   []))
                    if len(f_annees) == 1 and len(f_mois) == 1:
                        sauvegarder_csv(f_annees[0], f_mois[0])
                        st.success(f"{len(assign_chq)} chèque(s) catégorisé(s). CSV sauvegardé ✅")
                    else:
                        st.success(f"{len(assign_chq)} chèque(s) catégorisé(s) et mémorisé(s).")
                else:
                    st.success(f"{len(assign_chq)} chèque(s) catégorisé(s). {nb_restant} transaction(s) restantes.")
                st.rerun()
        st.divider()
    ...
    if non_cat.empty and cheques.empty and vir_opaques.empty:
        ...
        st.success("✅ Tous les libellés auto sont catégorisés. Traitez les sections ci-dessus si nécessaire.")
    else:
        ...
        st.info(f"{len(resume)} libellé(s) distinct(s) — validez ligne par ligne.")
        st.caption("💡 Chaque validation est immédiate : la nouvelle catégorie apparaît aussitôt pour les lignes suivantes.")
        st.divider()
        ...
        for _, rr in resume.iterrows():
            ...
            # Relire les catégories à chaque ligne pour avoir les nouvelles créées
            cats_dispo_live = sorted(st.session_state["categories"].keys(), key=str.lower)
            ...
            with st.container(border=True):
                ...
                with col_cat:
                    choix = st.selectbox(
                        "Catégorie",
                        ["— Choisir —"] + cats_dispo_live + ["➕ Nouvelle catégorie…"],
                        key=f"sel_{libelle}",
                        label_visibility="collapsed",
                    )
                    ...
                    if cat_choisie:
                        ...
                        mot_cle = st.text_input(
                            ...
                            help="Raccourcissez si besoin (ex : Netflix plutôt que Netflix.com Amsterdam).",
                            label_visibility="visible",
                        )

                        if st.button(f"✅ Valider", key=f"btn_val_{libelle}"):
                            cats = st.session_state["categories"]

                            # Créer la catégorie si nouvelle
                            if est_nouvelle and cat_choisie not in cats:
                                cats[cat_choisie] = []

                            # Enregistrer le mot-clé
                            mot_cle_final = (mot_cle.strip() or tronc).upper()
                            mots_existants = [m.upper() for m in cats.get(cat_choisie, [])]
                            if mot_cle_final and mot_cle_final not in mots_existants:
                                cats.setdefault(cat_choisie, []).append(mot_cle_final)

                            # Appliquer au DataFrame — tous les libellés correspondants
                            # (y compris d'autres libellés non encore traités qui matchent)
                            appliquer_categorisation()

                            # Forcer la catégorie sur ce libellé exact
                            ...
                            # CSV auto si tout est catégorisé
                            df_apres = st.session_state["combined_df"]
                            cats_incompletes = {"À catégoriser", "À catégoriser (chèque)"}
                            nb_restant = df_apres["Catégorie"].isin(cats_incompletes).sum()
                            if nb_restant == 0:
                                ...
                                if len(f_annees) == 1 and len(f_mois) == 1:
                                    sauvegarder_csv(f_annees[0], f_mois[0])
                                st.session_state["_goto_tab"] = "analyse"
                                st.success("✅ Tout est catégorisé ! Passage à l'analyse…")
                            st.rerun()
